...
# Store quiz/answers/submitted per level to avoid mixing sets across levels
...
def generate_quiz(level: str, use_ai: bool):
    ...
    if use_ai and DEEPSEEK_API_KEY:
        pool = [v["meaning_en"] for v in vocab]
        for item in items:
            try:
                quiz.append(call_deepseek_generate(item["word"], item["pinyin"], level, pool))
            except Exception:
                quiz.append(build_one_local_q(item, vocab))
    else:
        for item in items:
            quiz.append(build_one_local_q(item, vocab))
    return quiz
...
with col_top:
    ...
    ss.level = st.radio(t("level_label"), LEVELS, index=LEVELS.index(ss.level), key="level")
...
if ss.view == "lesson":
    ...
    # Start quiz: generate set for current level, switch view, rerun
    ...
    if c0.button("🆕 New quiz set", use_container_width=True):
        ss.quiz_map[level] = generate_quiz(level, use_ai)
        ss.answers_map[level] = {}
        ss.submitted_map[level] = False
        st.rerun()

    if c1.button("♻️ Regenerate", use_container_width=True):
        ss.quiz_map[level] = generate_quiz(level, use_ai)
        ss.answers_map[level] = {}
        ss.submitted_map[level] = False
        st.rerun()

    if c2.button("🧹 Clear answers", use_container_width=True):
        ss.answers_map[level] = {}
        ss.submitted_map[level] = False
        st.rerun()

    if c3.button("↩️ Back to Lesson", use_container_width=True):
        ss.view = "lesson"
        st.rerun()

    st.divider()

    # If no set yet, instruct user
    if not qset:
        st.info("No quiz yet — click **New quiz set** or go to Lesson and click **Start Quiz**.")
    else:
        # Render questions (keys include level to avoid collisions)
        for i, q in enumerate(qset, start=1):
            st.markdown(f"**Q{i}. {q['q']}**")
            labels = [f"{k}. {txt}" for k, txt in q["opts"]]
            picked = st.radio(f"Answer_{level}_{i}", labels, key=f"{level}_q{i}",
                              index=None if f"{level}_q{i}" not in ss else None,
        ...
        # Submit / Show results
        cS, _ = st.columns([1, 3])
        if not ss.submitted_map[level] and cS.button(t("submit"), type="primary", use_container_width=False):
